...
export const formatDateTimeWithSeconds = (dateTimeString) => {
  ...
  // created_at comes from Postgres (UTC in this setup), so parse as UTC and render in local system time
  const localValue = new Date(`${normalized}Z`);
  if (Number.isNaN(localValue.getTime())) return String(dateTimeString);
...
export const calculateEventMetrics = (event) => {
  ...
  if (
    ...
    pending > 0 ||
    (totalBudget > 0 && remaining / totalBudget < 0.2)
  ) {
    status = "Warning";
  }
...
export const getExpenseDistribution = (events) => {
  ...
  events.forEach((event) => {
    event.categories.forEach((category) => {
      map.set(
        category.name,
        (map.get(category.name) || 0) + Number(category.allocated || 0),
      );
    });
  });
...
export const buildCategoryReport = (events, eventId = "all") => {
  const selectedEvents =
    eventId === "all" ? events : events.filter((event) => event.id === eventId);
