...
const findColumn = (columnsSet, candidates) =>
  candidates.find((column) => columnsSet.has(column)) || null;
...
async function getSchemaMap() {
  ...
  const eventsMap = {
    ...
    statusColumn: findColumn(events, ["status", "event_status"]),
    closedAtColumn: findColumn(events, ["closed_at"]),
    closedByColumn: findColumn(events, ["closed_by"]),
    closingNoteColumn: findColumn(events, ["closing_note"]),
  };
  ...
  const fundingMap = {
    ...
    createdAtColumn: findColumn(funding, ["created_at"]),
  ...
  const categoriesMap = {
    ...
    paidColumn: findColumn(categories, ["paid", "paid_amount"]),
    createdAtColumn: findColumn(categories, ["created_at"]),
  ...
  assertColumn(
    usersMap.usernameColumn,
    "users table must have username column",
  );
  assertColumn(
    usersMap.passwordColumn,
    "users table must have password_hash column",
  );
  ...
  assertColumn(
    fundingMap.idColumn,
    "funding_sources table must have funder_id or id",
  );
  assertColumn(
    fundingMap.eventColumn,
    "funding_sources table must have event/event_id column",
  );
  assertColumn(
    fundingMap.nameColumn,
    "funding_sources table must have name column",
  );
  assertColumn(
    fundingMap.amountColumn,
    "funding_sources table must have amount column",
  );

  assertColumn(
    categoriesMap.idColumn,
    "categories table must have category_id or id",
  );
  assertColumn(
    categoriesMap.eventColumn,
    "categories table must have event/event_id column",
  );
  assertColumn(
    categoriesMap.nameColumn,
    "categories table must have name column",
  );
  assertColumn(
    categoriesMap.amountColumn,
    "categories table must have amount or allocated column",
  );
