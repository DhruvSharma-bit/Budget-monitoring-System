...
async function me(req, res) {
  ...
  const query = `
    SELECT *
    FROM ${quoteIdent(users.table)}
    WHERE ${quoteIdent(users.idColumn)}::text = $1
    LIMIT 1
  `

  const result = await db.query(query, [String(req.user.sub)])
  const userRow = result.rows[0]
  const role = userRow
    ? normalizeRole(users.roleColumn ? userRow[users.roleColumn] : null, userRow[users.usernameColumn])
    : req.user.role
  ...
  return res.json({
    ...
    data: {
      id: req.user.sub,
      name: userRow ? userRow[users.nameColumn] || userRow[users.usernameColumn] : req.user.name,
      email: userRow ? userRow[users.usernameColumn] : req.user.email,
      role,
    },
  })
}
